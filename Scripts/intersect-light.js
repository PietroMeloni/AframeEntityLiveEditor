...
var ALintersection = false;
var ALtransformCreated = false; //flag creazione transform (evita che venga creato più di una volta)
var targetObject = {
    aframeEl: null
}; //oggetto puntato
var ALcontrols = ['color', 'spot'];
var ALcurrentControl = 0;
var lightBody = null; //corpo visibile della luce in manipolazione

//mano selezionata tramite componente
function ALselectedHand(hand) {
...
//riconoscimento posa
function ALgestureRecognizer(hand) {
...
AFRAME.registerComponent('intersect-light', {
    ...
    tick: function () {
        ...
        if (transform !== null) { //non vale per editor Spot
            ...
            if (ALcontrols[ALcurrentControl] === "color"){
            ...
            } else if(ALcontrols[ALcurrentControl] === "spot"){
    ...
    raycasterIntersection: function (event) {
        ...
        //mano visibile
        let isVisible = ALselectedHand(event.target.components['intersect-light'].data.hand).components['leap-hand'].isVisible;
        if (isVisible) {
            if (intersectedObject.getAttribute(this.data.tag) !== null) {
                ...
                createTransform(ALcontrols[ALcurrentControl]);
                ALtransformCreated = true;

...
let IAMintersection = false;
let IAMtransformCreated = false; //flag creazione transform (evita che venga creato più di una volta)
let IAMtargetObject = {
    aframeEl: null
}; //oggetto puntato
let oldPosition = null;
let oldOpacity = null;
let IAMcontrols = ['translate', 'scale', 'rotate'];
let IAMcurrentControl = 0;

//mano selezionata tramite componente
function IAMselectedHand(hand) {
...
//riconoscimento posa
function IAMgestureRecognizer(hand) {
...
function createControl(transform, values) {
    console.log("sono entrato in createcontrol")
    let x, y, z, all;
...
function IAMcreateTransform(transformType) {
    console.log("sono entrato in createTransform")
    let values = null;
    ...
    transform.setAttribute('position', IAMtargetObject.aframeEl.getAttribute('position'));
...
AFRAME.registerComponent('intersect-and-manipulate', {
    ...
    raycasterIntersection: function (event) {
        ...
        //mano visibile
        let isVisible = IAMselectedHand(event.srcElement.components['intersect-and-manipulate'].data.hand).components['leap-hand'].isVisible;
        ...
        if (isVisible) {
            ...
            if (intersectedObject.getAttribute(this.data.tag) !== null) {
                ...
                intersectedObject.addEventListener('movingstarted', function (event) {
                    IAMtransformCreated = false;
                });
                intersectedObject.addEventListener('movingended', function (event) {
                    if (!IAMtransformCreated) {
                        //propagazione evento
                        event.srcElement.setAttribute('alongpath', {
                            triggerRadius: 0
                        });
                        event.srcElement.removeAttribute('alongpath');
                        if(IAMtargetObject.aframeEl !== null && targetObject.aframeEl !== undefined && oldOpacity !== null) {
                            IAMtargetObject.aframeEl.setAttribute('material', 'opacity: ' + oldOpacity);
                            //se l'elemento non è stato traslato
                            if(oldPosition !== null)
                                IAMtargetObject.aframeEl.setAttribute('position', oldPosition);
                        }
                        //aggiornamento vecchia posizione
                        oldPosition = endPath;
                        IAMtargetObject.aframeEl = event.srcElement;
                        if(IAMtargetObject.aframeEl.getAttribute('material') !== null)
                            oldOpacity = IAMtargetObject.aframeEl.getAttribute('material').opacity;
                        else
                            oldOpacity = null;
                        //creazione transform
                        IAMcreateTransform(IAMcontrols[IAMcurrentControl]);
                        IAMtransformCreated = true;
                        event.srcElement.setAttribute('material', 'opacity: 0.5');
                        updateEntireLateralBar(IAMtargetObject.aframeEl);
                    }
                });
            } else
                IAMintersection = false;
